...
def optimize_single(
    ...
    model = PICNN(input_dim=INPUT_DIM[ds], y_dim=Y_DIM[ds], hidden_dim=hidden_dim,
                  n_layers=n_layers, y_in_output_layer=Y_IN_OUTPUT_LAYER, gamma=GAMMA)
...
def main(args: argparse.Namespace) -> None:
    ...
    if ds == 'synthetic':
        ...
        if shuffle:
            ...
            SAVED_LR = None  # {a: 0.001 for a in (0.01, 0.05, 0.1, 0.2)}
            SAVED_L2REG = None  # {0.01: 1e-3, 0.05: 1e-4, 0.1: 1e-3, 0.2: 1e-3}
        else:
